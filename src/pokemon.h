...
class Pokemon final : public Creature
{
	public:
		...
		void onWalk(Direction& direction) override;
		...
		int8_t getBattleStatStage(PokemonBattleStat_t stat) const {
			return stat < POKEMON_BATTLE_STAT_COUNT ? battleStatStages[stat] : 0;
		}
		uint32_t getFlinchRemaining() const;
		...
		bool orderStep(Direction direction, uint8_t viewportX, uint8_t viewportY);
		void orderStay();
		void orderFollow();
	...
	private:
		...
		uint8_t orderViewportX = 7;
		uint8_t orderViewportY = 5;
