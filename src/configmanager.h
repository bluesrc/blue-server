...
class ConfigManager
{
	public:
		...
		enum boolean_config_t {
			...
			EMOTE_MOVES,
			STAMINA_SYSTEM,
			STAMINA_BONUS_PREMIUM_ONLY,
			STAMINA_LOW_STOPS_LOOT,
			WARN_UNSAFE_SCRIPTS,
			...
			REMOVE_ON_DESPAWN,
			PLAYER_CONSOLE_LOGS,
			ENABLE_CLIENT_ACCOUNT_CREATION,
			ENABLE_CLIENT_CHARACTER_CREATION,
		...
		enum integer_config_t {
			...
			STAMINA_MAX_HOURS,
			STAMINA_BONUS_HOURS,
			STAMINA_BONUS_EXPERIENCE_PERCENT,
			STAMINA_LOW_HOURS,
			STAMINA_LOW_EXPERIENCE_PERCENT,
			RATE_SKILL,
			RATE_LOOT,
			RATE_SPAWN,
			HOUSE_PRICE,
		...
		uint16_t getStaminaMaxMinutes() const;
