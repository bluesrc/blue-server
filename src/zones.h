...
class Zones
{
	public:
		...
		size_t getPokemonCount()							const { return getCount(CREATURETYPE_POKEMON); }
		...
		const CreatureVector getPokemons()								const { return getCreaturesByType(CREATURETYPE_POKEMON); }
	...
	private:
		inline bool checkCreature(CreatureType_t dataType, const Creature* creature) const
		{
			return (dataType == CREATURETYPE_PLAYER && creature->getPlayer()) ||
				...
				(dataType == CREATURETYPE_POKEMON && creature->getPokemon());
		}
