...
void Moves::clearMaps(bool fromLua)
{
...
bool Moves::registerInstantLuaEvent(InstantMove* event)
{
	InstantMove_ptr instant { event };
	if (instant) {
		std::string words = instant->getWords();
		auto result = instants.emplace(instant->getWords(), std::move(*instant));
		if (!result.second) {
			std::cout << "[Warning - Moves::registerInstantLuaEvent] Duplicate registered instant move with words: " << words << std::endl;
		}
		return result.second;
	}
...
bool Moves::registerRuneLuaEvent(RuneMove* event)
{
	RuneMove_ptr rune { event };
	if (rune) {
		uint16_t id = rune->getRuneItemId();
		auto result = runes.emplace(rune->getRuneItemId(), std::move(*rune));
		if (!result.second) {
			std::cout << "[Warning - Moves::registerRuneLuaEvent] Duplicate registered rune with id: " << id << std::endl;
		}
		return result.second;
	}
...
CombatMove::CombatMove(Combat_ptr combat, bool needTarget, bool needDirection) :
