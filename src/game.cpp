                          ...
                          uint16_t spriteId, uint8_t fromStackPos, const Position& toPos, uint8_t count, Item* item, Cylinder* toCylinder)
{
	...
	if (item->getPokeball() && toPos.x != 0xFFFF) {
                                   ...
                                   Item* item, uint32_t count, Item** _moveItem, uint32_t flags /*= 0*/, Creature* actor/* = nullptr*/, Item* tradeItem/* = nullptr*/, const Position* fromPos /*= nullptr*/, const Position* toPos/*= nullptr*/)
{
	...
	if (ret == RETURNVALUE_NEEDEXCHANGE) {
		if (fromCylinder == toCylinder) {
			if (Player* inventoryPlayer = dynamic_cast<Player*>(fromCylinder)) {
				const int32_t sourceIndex = inventoryPlayer->getThingIndex(item);
				if (inventoryPlayer->swapPokeballs(sourceIndex, index)) {
					if (actorPlayer && fromPos && toPos) {
						g_events->eventPlayerOnItemMoved(actorPlayer, item, count, *fromPos, *toPos, fromCylinder, toCylinder);
							  ...
							  bool inBackpacks/* = false*/)
{
	...
	merchant->onPlayerTrade(player, onBuy, it.id, subType, amount, false, inBackpacks);
}
...
void Game::parsePlayerExtendedOpcode(uint32_t playerId, uint8_t opcode, const std::string& buffer)
{
	...
	if (opcode == BOX_EXTENDED_OPCODE) {
		...
		for (const char character : buffer) {
			...
			depotId = static_cast<uint16_t>((depotId * 10) + (character - '0'));
			if (depotId >= BOX_DEPOT_COUNT) {
				return;
			}
		}
...
int Game::getPokemonCount(uint16_t zoneId)
{
	auto it = m_gameZones.find(zoneId);
	if (it == m_gameZones.end()) {
		return 0;
	...
	const ZonesPtr& zonesPtr = it->second;
	return zonesPtr->getPokemonCount();
}
...
std::vector<Creature*> Game::getPokemonsInZone(uint16_t zoneId)
{
	auto it = m_gameZones.find(zoneId);
	if (it == m_gameZones.end()) {
		return std::vector<Creature*>();
