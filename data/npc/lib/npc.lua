...
function doNpcSellItem(cid, itemid, amount, subType, canDropOnMap, inBackpacks, backpack)
	...
	if ItemType(itemid):isStackable() then
		...
		return Player(cid):addItemEx(stuff, canDropOnMap) ~= RETURNVALUE_NOERROR and 0 or amount, 0
	end
	...
	if inBackpacks then
		...
		for i = 1, amount do
			...
			if table.contains({(ItemType(backpack):getCapacity() * b), amount}, i) then
				if Player(cid):addItemEx(container, canDropOnMap) ~= RETURNVALUE_NOERROR then
					b = b - 1
					break
				end
	...
	for i = 1, amount do -- normal method for non-stackable items
		...
		if Player(cid):addItemEx(item, canDropOnMap) ~= RETURNVALUE_NOERROR then
			break
		end
		a = i
	end
